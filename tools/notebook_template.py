...
TEMPLATE = {
    "package": "convnextv2_classification_pipeline",
    "repo_name": "convnextv2-classification-pipeline",
    "stem": "convnextv2_classification",
    "notebook_name": "convnextv2_classification_colab.ipynb",
    "profile": "E2E",
    "mode": "GUIDED",
    "pipeline_class": "ConvNextV2Pipeline",
    "weights_key": "convnextv2-tiny-1k-224",
    "modules": [
        "data.py",
        "pipeline.py",
    ],
    ...
    "title": "ConvNeXt V2 Tiny (ImageNet-1k, 224 px) — DIMER image classification and bounded fine-tuning (standalone)",
    "badges": [
        (
            "GitHub",
            "https://img.shields.io/badge/GitHub-181717?style=flat&logo=github&logoColor=white",
            "https://github.com/kurtvalcorza/convnextv2-classification-pipeline",
        ),
        (
            "Open In Colab",
            "https://colab.research.google.com/assets/colab-badge.svg",
            "https://colab.research.google.com/github/kurtvalcorza/convnextv2-classification-pipeline/blob/main/tutorials/convnextv2_classification_colab.ipynb",
        ),
        (
            "Hugging Face",
            "https://img.shields.io/badge/%F0%9F%A4%97%20Hugging%20Face-facebook%2Fconvnextv2--tiny--1k--224-ffcc4d?style=flat",
            "https://huggingface.co/facebook/convnextv2-tiny-1k-224",
        ),
        (
            "Upstream",
            "https://img.shields.io/badge/Upstream-facebookresearch%2FConvNeXt--V2-181717?style=flat&logo=github&logoColor=white",
            "https://github.com/facebookresearch/ConvNeXt-V2",
        ),
        (
            "arXiv",
            "https://img.shields.io/badge/arXiv-2301.00808-b31b1b.svg",
            "https://arxiv.org/abs/2301.00808",
        ),
        (
            "License",
            "https://img.shields.io/badge/License-Apache--2.0-green.svg",
            "https://github.com/kurtvalcorza/convnextv2-classification-pipeline/blob/main/LICENSE",
        ),
    ],
    "capability": "ImageNet-1k image classification with ConvNeXt V2 Tiny, a pure convolutional network pre-trained as a masked autoencoder, and a bounded fine-tune that replaces the 1000-class head with one for your own classes, compares it on a held-out split with the majority-class and zero-shot ImageNet baselines, and exports a reloadable SafeTensors adapter",
    "intro": (
        "ConvNeXt V2 is a pure convolutional network (ConvNet). It was first pre-trained without labels as a fully convolutional masked "
        "autoencoder (FCMAE), reconstructing masked-out image patches, and then fine-tuned on ImageNet-1k at 224×224; it adds a Global "
        "Response Normalization (GRN) layer to each ConvNeXt block. The Tiny model has four stages of 3, 3, 9 and 3 blocks at widths 96, 192, "
        "384 and 768, about 28.6M parameters, and a linear head on the pooled 768-channel features that outputs a softmax over the 1000 "
        "ImageNet classes. Preprocessing follows the checkpoint's `ConvNextImageProcessor`: bicubic resize of the shorter side to 256 px, a "
        "224 px centre crop, and the ImageNet mean and standard deviation.\n\n"
        "**Licence note.** The Hugging Face card for `facebook/convnextv2-tiny-1k-224` is tagged Apache-2.0, but the upstream `ConvNeXt-V2` "
        "repository releases its ImageNet pre-trained and fine-tuned models under CC-BY-NC-4.0, a non-commercial licence. This notebook "
        "treats the weights, and any adapter fine-tuned from them, as CC-BY-NC-4.0.\n\n"
    ...
    "prerequisites": [
        "- **Runtime:** a fresh supported runtime (Google Colab or Jupyter, Python 3.12). A CUDA GPU such as a Colab or Kaggle T4 is the documented runtime for the fine-tuning stages and is used automatically when present; the notebook also runs on CPU, more slowly. Runtimes are not measured in this revision. The pinned `torch==2.14.0` wheel is the largest download; the checkpoint is about 115 MB.",
        "- **Knowledge:** basic Python and PIL; what a softmax over classes is; how a convolutional network pools features into a classifier; accuracy, balanced accuracy and a confusion matrix; why a baseline is needed before a score means anything.",
    ...
    "cells": [
        ...
        # ---------------------------------------------------------------- 7. Re-head & baseline
        {
            "md": (
                ...
                "(`classifier`, 768 features to 1000 classes) with a new 768-to-2 layer initialised under `SEED`. Every other weight keeps its "
                "ImageNet value.\n\n"
            ...
            "code": (
                "adapter = ConvNextV2Pipeline.from_pretrained(weights_dir=WEIGHTS_DIR, class_names=SAMPLE_CLASSES, seed=SEED)\n"
        ...
        # ---------------------------------------------------------------- 8. Fine-tune
        {
            "md": (
                ...
                "the network (`convnextv2.`) keeps its weights; ConvNeXt V2 normalises with LayerNorm and GRN, so there are no running statistics to hold. The cell "
        ...
        # ---------------------------------------------------------------- 11. Export, reload & verify
        {
            "md": (
                ...
                "`save_artifact` writes `outputs/convnextv2_adapter.safetensors`: every tensor the fine-tune could change, plus a metadata header "
            ...
            "code": (
                "artifact_path = OUTPUTS / 'convnextv2_adapter.safetensors'\n"
                "descriptor = adapter.save_artifact(artifact_path, notes='ConvNeXt V2 Tiny CIFAR-10 frog/truck tutorial adapter')\n"
                "print(json.dumps(descriptor, indent=2))\n\n"
                "reloaded = ConvNextV2Pipeline.load_artifact(artifact_path, weights_dir=WEIGHTS_DIR)\n"
        ...
        # ---------------------------------------------------------------- 12. Outputs & provenance
        {
            "md": (
                ...
                "The cell writes:\n"
                "- `outputs/convnextv2_classification_input_manifest.json`\n"
                "- `outputs/convnextv2_classification_evaluation_report.json`\n"
                "- `outputs/convnextv2_classification_result.json` (identity, runtime versions, device, dataset provenance and manifest, split, "
                "baselines, fine-tuning configuration, held-out and unseen metrics, adapter descriptor and reload check)\n"
                "- `outputs/convnextv2_classification_predictions.csv`\n"
                "- `outputs/convnextv2_adapter.safetensors` (written in Section 11)"
            ),
            "code": (
                "import csv\n\n"
        ...
        # ---------------------------------------------------------------- 13. BYOD
        {
            ...
            "code": (
                ...
                "    byod_pipe = ConvNextV2Pipeline.from_pretrained(weights_dir=WEIGHTS_DIR, class_names=byod_names, seed=SEED)\n"
                ...
                "    byod_descriptor = byod_pipe.save_artifact(OUTPUTS / 'byod_convnextv2_adapter.safetensors', notes='BYOD adaptation adapter')\n"
                "    ConvNextV2Pipeline.load_artifact(OUTPUTS / 'byod_convnextv2_adapter.safetensors', weights_dir=WEIGHTS_DIR)\n"
                "    print('BYOD adapter exported and reloaded:', byod_descriptor['sha256'][:16])\n"
                "else:\n"
                "    print('BYOD dataset branch is off; set USE_BYOD_DATASET = True to adapt ConvNeXt V2 Tiny on your own class folders.')"
            ),
        },
    ],
    "closing": (
        "## Interpretation and limits\n\n"
        "**What this notebook established, in this runtime.** The pinned `facebook/convnextv2-tiny-1k-224` snapshot was verified against a committed "
        ...
        "## References\n\n"
        "- Woo, S., Debnath, S., Hu, R., Chen, X., Liu, Z., Kweon, I. S. and Xie, S. (2023). *ConvNeXt V2: Co-designing and Scaling ConvNets with Masked Autoencoders.* CVPR 2023. [arXiv:2301.00808](https://arxiv.org/abs/2301.00808).\n"
        "- Hugging Face checkpoint: [facebook/convnextv2-tiny-1k-224](https://huggingface.co/facebook/convnextv2-tiny-1k-224) — tagged Apache-2.0 on the Hub; code and weights: [facebookresearch/ConvNeXt-V2](https://github.com/facebookresearch/ConvNeXt-V2) — MIT code, CC-BY-NC-4.0 ImageNet models.\n"
        ...
        "- Repository model card: https://github.com/kurtvalcorza/convnextv2-classification-pipeline/blob/main/MODEL_CARD.md\n"
        "- [`kurtvalcorza/convnextv2-classification-pipeline`](https://github.com/kurtvalcorza/convnextv2-classification-pipeline) — source repository for this pipeline."
    ),
}
