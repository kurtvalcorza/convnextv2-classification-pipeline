...
from convnextv2_classification_pipeline import pipeline as pipeline_module
...
def test_identity_constants_agree_with_the_committed_manifest():
    ...
    assert MODEL_ID == "facebook/convnextv2-tiny-1k-224" == manifest["modelId"]
    assert manifest["revision"] == pipeline_module.MODEL_REVISION
    assert is_pinned() == bool(HEX40.match(pipeline_module.MODEL_REVISION))
    assert DEFAULT_WEIGHTS_DIR == SNAPSHOT and ARTIFACT_FORMAT == "convnextv2-adapter-v1"
    assert pipeline_module.MODEL_LICENSE == "cc-by-nc-4.0"  # upstream weights licence, not the Hub tag
...
def test_pipeline_transform_matches_the_snapshot_image_processor():
    ...
    hub = json.loads(raw)
    assert (hub["size"], hub["crop_pct"], hub["resample"]) == ({"shortest_edge": 224}, 0.875, 3)
...
def test_loader_strict_loads_a_full_size_checkpoint_and_refuses_drift(tmp_path):
    ...
    assert sum(p.numel() for p in model.parameters()) == 28_635_496  # the paper's 28.6M for ConvNeXt V2 Tiny
    model.config.architectures = ["ConvNextV2ForImageClassification"]
    ...
    config["depths"] = [3, 3, 27, 3]
    (tmp_path / "config.json").write_text(json.dumps(config))
    with pytest.raises(ValueError, match="does not describe ConvNeXt V2 Tiny"):
        pipeline_module._load_pretrained(tmp_path)
