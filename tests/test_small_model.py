"""End-to-end adaptation on a small random-weight ConvNeXt V2: no checkpoint, no network.

The model is the same ``ConvNextV2ForImageClassification`` class the pinned checkpoint loads into,
shrunk to one narrow block per stage on 32 px inputs so a CPU step takes well under a second. These tests
...
from convnextv2_classification_pipeline import (  # noqa: E402
    ARTIFACT_FORMAT,
    IMAGENET_GROUPS,
    MODEL_ID,
    MODEL_KEY,
...
from convnextv2_classification_pipeline import pipeline as pipeline_module  # noqa: E402

imagenet_labels = pipeline_module.imagenet_labels


SMALL = {"hidden_sizes": [8, 16, 24, 32], "depths": [1, 1, 1, 1]}


def _imagenet_pipeline(seed: int = 0) -> ConvNextV2Pipeline:
    torch.manual_seed(seed)
    model = build_model(**SMALL)
    return ConvNextV2Pipeline.from_components(
        model, class_names=imagenet_labels(model), transform=pipeline_module.eval_transform(image_size=32)
    )


def _adapter_pipeline(seed: int = 0, class_names=SAMPLE_CLASSES) -> ConvNextV2Pipeline:
...
def test_backbone_is_everything_but_the_classifier():
    assert pipeline_module._backbone_prefixes(build_model(**SMALL)) == ("convnextv2.",)
