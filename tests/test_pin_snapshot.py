...
FILES = {
    "README.md": b"# card\n",
    "config.json": b'{"model_type": "convnextv2"}',
...
def test_pin_writes_commit_digests_and_module_revision(tmp_path):
    ...
    manifest = json.loads((root / "weights/convnextv2-tiny-1k-224/dimer-base-manifest.json").read_text())
    ...
    module = (root / "src/convnextv2_classification_pipeline/pipeline.py").read_text()
...
def test_pin_refuses_a_digest_that_disagrees_with_the_hub(tmp_path):
    root = _copy_repo(tmp_path)
    before = (root / "weights/convnextv2-tiny-1k-224/dimer-base-manifest.json").read_text()
    module_before = (root / "src/convnextv2_classification_pipeline/pipeline.py").read_text()
    ...
    assert (root / "weights/convnextv2-tiny-1k-224/dimer-base-manifest.json").read_text() == before
    assert (root / "src/convnextv2_classification_pipeline/pipeline.py").read_text() == module_before
