...
Notebook identity is the Git blob id of `tutorials/convnextv2_classification_colab.ipynb` (verify with `git rev-parse <commit>:tutorials/convnextv2_classification_colab.ipynb`). Each record uses the fields Date, Subject, Runtime, Procedure, Observed result and Caveats.
