# ConvNeXt V2 Tiny image classification pipeline

DIMER pipeline for **ConvNeXt V2 Tiny** (`facebook/convnextv2-tiny-1k-224`), a pure convolutional network pre-trained as a masked autoencoder and fine-tuned on ImageNet-1k.
...
- Preprocessing: the snapshot's `ConvNextImageProcessor` settings (bicubic resize to 256 px, centre crop 224, ImageNet mean/std), re-implemented with torchvision and tested against the processor
...
```
weights/convnextv2-tiny-1k-224/
  dimer-base-manifest.json   # modelId, revision, per-file bytes + SHA-256 (4 files + 2 reference files)
  preprocessor_config.json   # committed; ConvNextImageProcessor, shortest edge 224, crop_pct 0.875
  config.json                # staged; ConvNextV2ForImageClassification, depths 3-3-9-3, widths 96-768
  model.safetensors          # git-ignored, 114,565,784 bytes; the executed artifact
...
This repository's code is Apache-2.0 (see `LICENSE`). The upstream weights are CC-BY-NC-4.0 (non-commercial) according to the upstream repository, although the Hub card is tagged Apache-2.0, and the tutorial's sample archive is MIT; see `docs/WEIGHTS.md` and `MODEL_CARD.md`.
